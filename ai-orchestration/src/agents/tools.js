...
export const listFiles = tool(
    async ({ }, config) => {
        ...
        const writer = config.context?.writer ?? (() => {});  // here writer is a callback function that is used to write the output of the tool to the console
...
export const readFiles = tool(
    async ({ files = [] }, config) => {
        ...
        writer("Reading files..." + files.join(",") + "\n"); // to show the log of all files that is being read
